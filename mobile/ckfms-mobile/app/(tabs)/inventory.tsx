    ...
    try {
      ...
      try {
        ...
        if (userData?.store) {
          ...
          setStoreInfo({
            name: "Tồn kho cửa hàng",
            description: `${storeName} - ${areaName}`,
          });
        }
      } catch (e) {
      ...
      try {
        ...
        if (userStoreId) {
          res = await api.get(`/store/inventory/${userStoreId}`, {
            params: { per_page: 50 },
          });
        } else {
          throw e;
      ...
      data = data.map((item: any) => ({
        ...
        // Used for client-side merge (same item may exist in multiple warehouses)
        item_id: item.item_id ?? item.item?.id ?? item.id,
      }));

      // Gộp trùng theo item_id để tránh hiển thị 2 dòng cùng 1 mặt hàng
      ...
      for (const row of data) {
        ...
        if (!merged.has(key)) {
          merged.set(key, { ...row, id: key, quantity: qty });
        } else {
          const prev = merged.get(key);
          merged.set(key, {
            ...prev,
            quantity: (Number(prev.quantity) || 0) + qty,
            // keep the first min_quantity/unit/name for the same item
          });
        }
      }

      setInventory(Array.from(merged.values()));
    } catch (err) {
      console.error(err);
